Въвеждане на отрицателни стойности НЯМА: 
READ n
няма да прочете -3 примерно

компилира се с:
g++ -o main main.cpp
интерпретира се произволна програма с:
./main <example_program_name>
даже не е задължено някакво разширение - сложил съм ги програмите ми *.lu за красота (идва от фамилното ми име)

МОЖЕ да се въвеждат няколко променливи на едни ред:
READ a
READ b
READ c
безпроблемно ще прочете
1 2 3
и ще направи 
a = 1, b = 2, c = 3

Тема 1: Интерпретатор.

Да се напише интерпретатор на езика, определен със запазените думи:
LАBEL, GOTO, LET, READ, PRINT, IF, ENDIF, ELSE, WHILE, DONE
И оператори =, ==, !=, <, <=, >, >=, +, *, /, -, %, &&, ||, !,()
работещ над целочислени данни и масиви от такива. Масивите се дефинират с конкретен размер. Операторите имат синтаксис и семантика, идентични със C++, като можете да построявате произволно сложни изрази. Идентификаторите също са със синтаксис на C++.
